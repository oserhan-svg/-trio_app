        ...
        try {
            ...
            if (page === 1 && results[1]) {
                ...
                if (analyticsResult.status === 'fulfilled') {
                    // Fix: Backend returns object { totalProperties, marketStats, supplyDemand }
                    // We need to pass the array to NeighborhoodChart (which expects array)
                    // and store totalProperties for header.
                    setStats(analyticsResult.value.data.marketStats || []);
                    // We might need totalProperties separately if used elsewhere
                    if (analyticsResult.value.data.totalProperties) {
                        setMeta(prev => ({ ...prev, total: analyticsResult.value.data.totalProperties }));
                    }
                } else {
            ...
            <div className="min-h-screen bg-gray-100 pb-20 md:pb-0"> {/* Added padding bottom for mobile if needed */}
                ...
                <nav className="hidden md:flex bg-white shadow-sm px-6 py-4 justify-between items-center sticky top-0 z-50">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button onClick={() => navigate('/consultant-panel')} className="flex items-center gap-1 text-gray-600 hover:text-blue-600 font-medium border-r pr-4 mr-4">
                            <Users size={18} />
                            <span className="hidden md:inline">Danışman Paneli</span>
                        </button>
                        {user?.role === 'admin' && (
                            <button onClick={() => navigate('/admin/management')} className="flex items-center gap-1 text-gray-600 hover:text-purple-600 font-medium">
                                <Settings size={18} />
                                <span className="hidden md:inline">Yönetim Paneli</span>
                            </button>
                        )}
                        <div className="text-sm text-gray-500 hidden md:flex flex-col items-end leading-none">
                            ...
                            <span className="text-[10px] opacity-70">veri tabanında {meta.total} kayıt</span>
                        </div>
