...
const Button = ({ children, variant = 'primary', className = '', isLoading = false, disabled, ...props }) => {
    const baseStyles = 'inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl transition-all duration-200 font-medium focus:outline-none focus:ring-2 focus:ring-offset-2';
    ...
    return (
        <button
            className={`${baseStyles} ${variants[variant]} disabled:opacity-70 disabled:cursor-not-allowed ${className}`}
