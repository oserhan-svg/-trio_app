                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                    <div>
                        ...
                        <div className="flex gap-2 items-center">
                            <label className={`flex-1 cursor-pointer bg-emerald-50 hover:bg-emerald-100 text-emerald-600 border border-emerald-200 border-dashed rounded-xl p-3 text-center transition-colors ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                <span className="text-sm font-medium">{uploading ? 'Yükleniyor...' : 'Dosya Seç & Yükle (PDF/Resim)'}</span>
                                <input type="file" className="hidden" onChange={handleFileUpload} accept=".pdf,.jpg,.jpeg,.png" disabled={uploading || saving} />
                            </label>
                        </div>
                        {formData.auth_doc_url && (
                            <div className="mt-2 text-xs text-emerald-600 flex items-center gap-1 bg-emerald-50 p-2 rounded-lg border border-emerald-100">
                                <FileText size={12} />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                type="date"
                                required
                                className="w-full text-sm border-gray-300 rounded-xl focus:ring-emerald-500 focus:border-emerald-500 shadow-sm"
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
                        ...
                        <Button type="submit" isLoading={saving} loadingText="Kaydediliyor..." disabled={uploading}>Kaydet</Button>
                    </div>
                </form>
            </div>
        </div>
