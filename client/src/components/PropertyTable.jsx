...
const PropertyTable = ({ properties, currentSort, onSortChange, hasMore, onLoadMore, isLoadingMore }) => {
    ...
    const SortHeader = ({ label, sortKeyBase, currentSort, onSortChange }) => {
        ...
        return (
            <th
                className={`px-6 py-5 text-left text-[10px] font-black uppercase tracking-[0.2em] transition-all relative group/th ${isCurrent ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-slate-200'}`}
            >
                <button
                    onClick={handleClick}
                    className="flex items-center gap-2 w-full h-full text-left uppercase tracking-[0.2em] focus:outline-none focus:ring-2 focus:ring-blue-500 rounded"
