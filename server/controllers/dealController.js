...
const runInternalMigration = async (req, res) => {
    ...
    exec(command, (error, stdout, stderr) => {
        if (error) {
            ...
            // SEC: Sanitize error output to prevent leaking internal stack traces or environment details
            return res.status(500).json({ error: 'Migration failed' });
        }
        // SEC: Sanitize success output to prevent leaking internal environment details
