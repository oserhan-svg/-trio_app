...
const runInternalMigration = async (req, res) => {
    ...
    exec(command, (error, stdout, stderr) => {
        if (error) {
            ...
            // 🛡️ Sentinel: Sanitize error response to prevent leaking internal stack traces and command output
            return res.status(500).json({ error: 'Migration failed. Please check server logs for details.' });
        }
        // 🛡️ Sentinel: Do not expose raw command output to the client
