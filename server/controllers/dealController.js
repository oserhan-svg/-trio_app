...
const runInternalMigration = async (req, res) => {
    ...
    exec(command, (error, stdout, stderr) => {
        if (error) {
            ...
            // 🛡️ Sentinel: Sanitized error response to prevent leaking internal stack traces or environment details (stderr)
            return res.status(500).json({ error: 'Migration failed. Please check server logs.' });
        }
