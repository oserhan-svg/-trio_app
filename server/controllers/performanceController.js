...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        const monthlyStatsPromise = Promise.all(months.map(async (m) => {
        ...
        // Client distribution
        const clientStatusDistPromise = prisma.client.groupBy({
            by: ['status'],
        ...
        // Recent activities
        const recentInteractionsPromise = prisma.interaction.findMany({
