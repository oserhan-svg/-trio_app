    ...
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            const [saleCount, rentCount, newPortfolioCount, interactionCount, completedTasks] = await Promise.all([
                prisma.property.count({ where: { assigned_user_id: c.id, listing_type: 'sale' } }),
                prisma.property.count({ where: { assigned_user_id: c.id, listing_type: 'rent' } }),
                prisma.property.count({ where: { assigned_user_id: c.id, created_at: { gte: startOfMonth } } }),
                prisma.interaction.count({ where: { client: { consultant_id: c.id }, date: { gte: startOfMonth } } }),
                prisma.agendaItem.count({ where: { user_id: c.id, status: 'completed', start_at: { gte: startOfMonth } } })
            ]);
    ...
    try {
        ...
        const monthlyStats = await Promise.all(months.map(async (m) => {
            const [propertiesCount, interactionsCount] = await Promise.all([
                prisma.property.count({ where: { assigned_user_id: consultantId, created_at: { gte: m.start, lte: m.end } } }),
                prisma.interaction.count({ where: { client: { consultant_id: consultantId }, date: { gte: m.start, lte: m.end } } })
            ]);
        ...
        // Client distribution and Recent activities concurrently
        const [clientStatusDist, recentInteractions] = await Promise.all([
            prisma.client.groupBy({ by: ['status'], where: { consultant_id: consultantId }, _count: { id: true } }),
            prisma.interaction.findMany({ where: { client: { consultant_id: consultantId } }, orderBy: { date: 'desc' }, take: 10, include: { client: { select: { name: true } } } })
        ]);
