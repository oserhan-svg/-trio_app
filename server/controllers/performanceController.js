...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            // ⚡ Bolt: Execute independent counts concurrently to eliminate query waterfall per consultant
...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        const monthlyStats = await Promise.all(months.map(async (m) => {
            // ⚡ Bolt: Execute independent month stats concurrently
            const [propertiesCount, interactionsCount] = await Promise.all([
                prisma.property.count({
                    where: { assigned_user_id: consultantId, created_at: { gte: m.start, lte: m.end } }
                }),
                prisma.interaction.count({
                    where: { client: { consultant_id: consultantId }, date: { gte: m.start, lte: m.end } }
                })
            ]);
