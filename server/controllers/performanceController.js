...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            // ⚡ Bolt Optimization: Run all 5 independent database aggregations concurrently
            // instead of sequentially to reduce DB latency from O(5N) to O(N) where N is consultants.
            const [
                saleCount,
                rentCount,
                newPortfolioCount,
...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        const monthlyStats = await Promise.all(months.map(async (m) => {
            // ⚡ Bolt Optimization: Run independent metric aggregations concurrently per month
            // reducing latency from O(2M) to O(M) where M is the number of months.
