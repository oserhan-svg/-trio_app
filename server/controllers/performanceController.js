...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            // ⚡ Bolt: Execute independent queries concurrently to eliminate sequential latency bottlenecks
            // Expected Impact: Reduces database query latency per consultant from 5x RTT to ~1x RTT
            const [
                saleCount,
                rentCount,
                newPortfolioCount,
...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        const monthlyStats = await Promise.all(months.map(async (m) => {
            // ⚡ Bolt: Fetch monthly counts concurrently
        ...
        // ⚡ Bolt: Execute independent detail queries concurrently
