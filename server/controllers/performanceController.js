...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            // ⚡ Bolt: Batch independent queries concurrently to prevent sequential bottleneck
            const [
                saleCount,
                rentCount,
                newPortfolioCount,
...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        const monthlyStats = await Promise.all(months.map(async (m) => {
            // ⚡ Bolt: Batch independent counts concurrently
        ...
        // ⚡ Bolt: Fetch client distribution and recent activities concurrently
