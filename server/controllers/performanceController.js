...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            // ⚡ Bolt Optimization: Grouped independent count queries concurrently using Promise.all inside the consultant loop. Expected impact: Reduces loop latency significantly (e.g. from ~50ms to ~10ms per consultant mapping).
            const [
                saleCount,
                rentCount,
                newPortfolioCount,
...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        const monthlyStats = await Promise.all(months.map(async (m) => {
            // ⚡ Bolt Optimization: Parallelized independent count queries using Promise.all to avoid sequence bottlenecks.
        ...
        // ⚡ Bolt Optimization: Fetched independent analytical queries concurrently using Promise.all.
