...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            // ⚡ Bolt: Execute independent DB aggregations concurrently to reduce latency bottlenecks
            const [
                saleCount,
                rentCount,
                newPortfolioCount,
...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        // ⚡ Bolt: Execute independent queries and inner collection processing concurrently for maximum concurrency
