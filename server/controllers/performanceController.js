...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            // Batch all independent count queries to execute concurrently,
            // preventing a sequential N+1 bottleneck per consultant.
            const [
                saleCount,
                rentCount,
                newPortfolioCount,
...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        const monthlyStats = await Promise.all(months.map(async (m) => {
            // Batch independent count queries concurrently for performance
