...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        // Pre-calculate aggregate counts for all consultants to avoid N+1 query problem
        const [sales, rents, newPortfolios, agendaTasks, interactionsList] = await Promise.all([
            prisma.property.groupBy({
                by: ['assigned_user_id'],
                where: { assigned_user_id: { not: null }, listing_type: 'sale' },
                _count: { _all: true }
            }),
            prisma.property.groupBy({
                by: ['assigned_user_id'],
                where: { assigned_user_id: { not: null }, listing_type: 'rent' },
                _count: { _all: true }
            }),
            prisma.property.groupBy({
                by: ['assigned_user_id'],
            ...
            prisma.agendaItem.groupBy({
                by: ['user_id'],
                where: { user_id: { not: null }, status: 'completed', start_at: { gte: startOfMonth } },
                _count: { _all: true }
            }),
            prisma.interaction.findMany({
                where: { client: { consultant_id: { not: null } }, date: { gte: startOfMonth } },
                select: { client: { select: { consultant_id: true } } }
            })
        ]);

        // Transform arrays to lookup maps for O(1) access
        const countMap = (arr, key) => arr.reduce((acc, item) => {
            if (item[key]) acc[item[key]] = item._count._all;
            return acc;
        }, {});

        const saleMap = countMap(sales, 'assigned_user_id');
        const rentMap = countMap(rents, 'assigned_user_id');
        const newPortfolioMap = countMap(newPortfolios, 'assigned_user_id');
        const tasksMap = countMap(agendaTasks, 'user_id');

        // Interactions need manual reduction due to nested relation grouping limitations
        const interactionMap = interactionsList.reduce((acc, item) => {
            const cId = item.client?.consultant_id;
