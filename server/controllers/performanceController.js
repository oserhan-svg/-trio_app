const prisma = require('../db');

exports.getConsultantPerformance = async (req, res) => {
    try {
        const consultants = await prisma.user.findMany({
            where: { role: 'consultant' },
            select: {
                id: true,
                email: true,
                _count: {
                    select: {
                        clients: true,
                        agenda_items: true,
                        properties: true
                    }
                },
                name: true
            }
        });

        // Current month date range
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);

        // OPTIMIZATION: Replace O(N) database queries per consultant with batched O(1) queries using Prisma groupBy and findMany
        const consultantIds = consultants.map(c => c.id);

        // 1. Group active sale/rent listings
        const propertyGroups = await prisma.property.groupBy({
            by: ['assigned_user_id', 'listing_type'],
            where: { assigned_user_id: { in: consultantIds } },
            _count: { _all: true }
        });

        // 2. Group new portfolios this month
        const newPortfolioGroups = await prisma.property.groupBy({
            by: ['assigned_user_id'],
            where: {
                assigned_user_id: { in: consultantIds },
                created_at: { gte: startOfMonth }
            },
            _count: { _all: true }
        });

        // 3. Batch fetch interactions and count locally (Prisma doesn't support grouping by relation fields directly)
        const recentInteractions = await prisma.interaction.findMany({
            where: {
                client: { consultant_id: { in: consultantIds } },
                date: { gte: startOfMonth }
            },
            select: { client: { select: { consultant_id: true } } }
        });

        const interactionCounts = {};
        for (const interaction of recentInteractions) {
            const cid = interaction.client?.consultant_id;
            if (cid) {
                interactionCounts[cid] = (interactionCounts[cid] || 0) + 1;
            }
        }

        // 4. Group completed agenda items
        const completedTasksGroups = await prisma.agendaItem.groupBy({
            by: ['user_id'],
            where: {
                user_id: { in: consultantIds },
                status: 'completed',
                start_at: { gte: startOfMonth }
            },
            _count: { _all: true }
        });

        // Map aggregated data back to consultants
        const performanceData = consultants.map((c) => {
            const sales = propertyGroups.find(p => p.assigned_user_id === c.id && p.listing_type === 'sale')?._count._all || 0;
            const rents = propertyGroups.find(p => p.assigned_user_id === c.id && p.listing_type === 'rent')?._count._all || 0;
            const newPortfolios = newPortfolioGroups.find(p => p.assigned_user_id === c.id)?._count._all || 0;
            const tasks = completedTasksGroups.find(t => t.user_id === c.id)?._count._all || 0;
            const interactions = interactionCounts[c.id] || 0;

            return {
                id: c.id,
                email: c.email,
                name: c.name,
                stats: {
                    total_clients: c._count.clients,
                    active_sale: sales,
                    active_rent: rents,
                    new_portfolio_monthly: newPortfolios,
                    interactions_monthly: interactions,
                    completed_tasks_monthly: tasks
                }
            };
        });

        res.json(performanceData);
    } catch (error) {
        console.error('Performance API Error:', error);
        res.status(500).json({ message: 'Performans verileri alınamadı.' });
    }
};

exports.getConsultantDetail = async (req, res) => {
    try {
        const { id } = req.params;
        const consultantId = parseInt(id);

        const now = new Date();
        const months = [];
        for (let i = 5; i >= 0; i--) {
            const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
            months.push({
                name: d.toLocaleString('tr-TR', { month: 'long' }),
                month: d.getMonth(),
                year: d.getFullYear(),
                start: new Date(d.getFullYear(), d.getMonth(), 1),
                end: new Date(d.getFullYear(), d.getMonth() + 1, 0, 23, 59, 59)
            });
        }

        const monthlyStats = await Promise.all(months.map(async (m) => {
            const propertiesCount = await prisma.property.count({
                where: {
                    assigned_user_id: consultantId,
                    created_at: { gte: m.start, lte: m.end }
                }
            });

            const interactionsCount = await prisma.interaction.count({
                where: {
                    client: { consultant_id: consultantId },
                    date: { gte: m.start, lte: m.end }
                }
            });

            return {
                name: m.name,
                portföy: propertiesCount,
                etkileşim: interactionsCount
            };
        }));

        // Client distribution
        const clientStatusDist = await prisma.client.groupBy({
            by: ['status'],
            where: { consultant_id: consultantId },
            _count: { id: true }
        });

        // Recent activities
        const recentInteractions = await prisma.interaction.findMany({
            where: { client: { consultant_id: consultantId } },
            orderBy: { date: 'desc' },
            take: 10,
            include: { client: { select: { name: true } } }
        });

        res.json({
            monthlyStats,
            clientStatusDist: clientStatusDist.map(d => ({ name: d.status, value: d._count.id })),
            recentInteractions
        });
    } catch (error) {
        console.error('Consultant Detail API Error:', error);
        res.status(500).json({ message: 'Detay verileri alınamadı.' });
    }
};
