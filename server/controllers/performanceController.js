...
exports.getConsultantPerformance = async (req, res) => {
    try {
        ...
        const performanceData = await Promise.all(consultants.map(async (c) => {
            // ⚡ Bolt: Fetch consultant performance metrics concurrently instead of sequentially to reduce latency bottleneck
            const [
                saleCount,
                rentCount,
                newPortfolioCount,
...
exports.getConsultantDetail = async (req, res) => {
    try {
        ...
        const monthlyStats = await Promise.all(months.map(async (m) => {
            // ⚡ Bolt: Fetch monthly stats concurrently instead of sequentially to reduce latency bottleneck
